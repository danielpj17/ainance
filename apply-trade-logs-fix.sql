-- Apply trade logs migration to fix trade logs not showing up
...
drop policy if exists "Users can insert own trade logs" on trade_logs;
create policy "Users can insert own trade logs" on trade_logs 
  for insert with check (auth.uid() = user_id);
  
drop policy if exists "Users can update own trade logs" on trade_logs;
...
drop trigger if exists trigger_update_trade_logs_updated_at on trade_logs;
...
-- Enable realtime for trade_logs (if publication exists)
do $$
begin
...
-- Verify the setup
select 'trade_logs table created successfully' as status;
select 'Functions created: get_current_trades, get_completed_trades, get_trade_statistics, close_trade_position' as functions;
