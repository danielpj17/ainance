 ...
 * Stock Names Utility
 * 
 * Provides company names for stock symbols
 */

// A curated list of popular stocks with their company names
// This matches the database in app/api/stocks/search/route.ts
...
STOCK_DATABASE.forEach(stock => {
  STOCK_MAP.set(stock.symbol.toUpperCase(), stock.name);
});
...
export function getCompanyName(symbol: string | null | undefined): string | null {
  ...
  return STOCK_MAP.get(symbol.toUpperCase()) || null;
}

/**
 * Get company name with fallback to symbol
 * @param symbol - The stock ticker symbol
 * @returns The company name if found, otherwise the symbol
 */
export function getCompanyNameOrSymbol(symbol: string | null | undefined): string {
  if (!symbol) return 'N/A';
  return getCompanyName(symbol) || symbol;
}

/**
 * Format symbol with company name
 * @param symbol - The stock ticker symbol
 * @returns Formatted string like "AAPL - Apple Inc." or just "AAPL" if name not found
 */
export function formatSymbolWithName(symbol: string | null | undefined): string {
  if (!symbol) return 'N/A';
